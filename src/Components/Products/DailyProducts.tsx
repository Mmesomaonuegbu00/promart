...
const DailyProducts = () => {
    ...
    const renderSkeletons = (count: number) => {
        return Array.from({ length: count }).map((_, i) => (
            <div
                key={i}
                className="animate-pulse bg-white p-3 rounded-md shadow border border-gray-200 flex flex-col gap-3 mb-6"
            >
        ...
        <div className='w-full '>
            <div className='w-[95%] lg:w-[85%] mx-auto'>
                ...
                <div className='mb-6'>
                    <h2 className='md:text-xl font-semibold text-center mb-8'>Top Picks</h2>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                        {loading ? renderSkeletons(5) : topPicks.map(p => renderProduct(p, true))}
                    </div>
                </div>
